...
public class TokensIT {
  ...
  private static String DUMMY_RS_SERVER = RandomStringUtils.randomAlphabetic(10).toLowerCase() + ".com";
  ...
  private static String providerAlphaToken; // has role for REAL_RS_URL and has items on dev catalogue
  ...
  /**
   * Validate exp and iat fields in the introspected token and make sure expiry time
   * matches the configured expiry.
   * 
  ...
  boolean validateExpAndIat(String introspectResp) {
    ...
    return obj.getValue("exp") instanceof Integer && obj.getValue("iat") instanceof Integer
  ...
  boolean validateCons(String introspectResp, boolean isEmpty) {
    JsonObject obj = new JsonObject(introspectResp).getJsonObject("results");
    return obj.getValue("cons") instanceof JsonObject && obj.getJsonObject("cons").isEmpty() == isEmpty;
  }
  ...
  void doesNotHaveAnyRoles(KcAdminInt kc)
  {
    ...
    JsonObject tokenReq = new JsonObject().put("itemId", DUMMY_RS_SERVER)
        .put("itemType", ItemType.RESOURCE_SERVER.toString().toLowerCase())
        .put("role", Roles.CONSUMER.toString().toLowerCase());

    given().auth().oauth2(noRolesToken)
    .contentType(ContentType.JSON).body(tokenReq.toString())
    .when().post("/token").then().statusCode(404)
  ...
  void doesNotHaveConsumerRole()
  {
    ...
    given().auth().oauth2(consumerRealRsToken)
    .contentType(ContentType.JSON).body(tokenReq.toString())
    .when().post("/token").then().statusCode(400)
    .body("type", equalTo(Urn.URN_INVALID_INPUT.toString()));
    
    tokenReq = new JsonObject().put("itemId", DUMMY_RS_SERVER)
        .put("role", Roles.CONSUMER.toString().toLowerCase());
    ...
    tokenReq = new JsonObject().put("itemId", RandomStringUtils.random(10) + ".com")
        .put("itemType", ItemType.RESOURCE_SERVER.toString().toLowerCase())
        .put("role", Roles.CONSUMER.toString().toLowerCase());
  ...
  class ConsumerTokenTests
  {
    ...
    void doesNotHaveConsumerRole()
    {
      ...
      given().auth().oauth2(providerAlphaToken)
      .contentType(ContentType.JSON).body(tokenReq.toString())
    ...
    void noResGrpToken()
    {
      JsonObject tokenReq = new JsonObject().put("itemId", ALPHA_RES_GROUP_ID)
    ...
    void noCosToken()
    {
      JsonObject tokenReq = new JsonObject().put("itemId", REAL_COS_URL)
    ...
    void resItemDoesNotExist()
    {
      ...
      JsonObject tokenReq = new JsonObject().put("itemId", badId)
    ...
    void resItemInvalid()
    {
      JsonObject tokenReq = new JsonObject().put("itemId", REAL_RS_URL)
          .put("itemType", ItemType.RESOURCE.toString().toLowerCase())
          .put("role", Roles.CONSUMER.toString().toLowerCase());
    ...
    {
      JsonObject tokenReq = new JsonObject().put("itemId", BRAVO_RES_GROUP_ID)
          .put("itemType", ItemType.RESOURCE.toString().toLowerCase())
          .put("role", Roles.CONSUMER.toString().toLowerCase());
    ...
    {
      JsonObject tokenReq = new JsonObject().put("itemId", BRAVO_RES_ITEM_SECURE_ID)
          .put("itemType", ItemType.RESOURCE.toString().toLowerCase())
          .put("role", Roles.CONSUMER.toString().toLowerCase());
      
      // consumerDummyRs has consumer role for DUMMY_SERVER and not REAL_RS - so call will fail
      given().auth().oauth2(consumerDummyRsToken)
      .contentType(ContentType.JSON).body(tokenReq.toString())
      .when().post("/token").then().statusCode(403)
      .body("type", equalTo(Urn.URN_INVALID_INPUT.toString()))
    ...
    {
      JsonObject tokenReq = new JsonObject().put("itemId", ALPHA_RES_ITEM_SECURE_ID)
          .put("itemType", ItemType.RESOURCE.toString().toLowerCase())
          .put("role", Roles.CONSUMER.toString().toLowerCase())
          .put("context", new JsonObject().put("TestAllow", true));
      
      String token = given().auth().oauth2(consumerRealRsToken)
      .contentType(ContentType.JSON).body(tokenReq.toString())
      ...
      .extract().path("results.accessToken");
      
      String introspectResp = given()
      .contentType(ContentType.JSON).body(new JsonObject().put("accessToken", token).toString())
      .when().post("/introspect").then().statusCode(200)
      .body("type", equalTo(Urn.URN_SUCCESS.toString()))
    ...
    void resItemApdDeny()
    {
      JsonObject tokenReq = new JsonObject().put("itemId", ALPHA_RES_ITEM_OPEN_ID)
          .put("itemType", ItemType.RESOURCE.toString().toLowerCase())
          .put("role", Roles.CONSUMER.toString().toLowerCase())
    ...
    {
      JsonObject tokenReq = new JsonObject().put("itemId", RandomStringUtils.randomAlphabetic(10) + ".com")
          .put("itemType", ItemType.RESOURCE_SERVER.toString().toLowerCase())
          .put("role", Roles.CONSUMER.toString().toLowerCase());
  ...
  class ProviderTokenTests
  {
    ...
    void resItemNoRoleForRs()
    {
      ...
      given().auth().oauth2(providerDummyRsToken)
      .contentType(ContentType.JSON).body(tokenReq.toString())
      .when().post("/token").then().statusCode(403)
      .body("type", equalTo(Urn.URN_INVALID_INPUT.toString()))
      .body("title", equalTo(ACCESS_DENIED))
    ...
    {
      JsonObject tokenReq = new JsonObject().put("itemId", BRAVO_RES_ITEM_PII_ID)
          .put("itemType", ItemType.RESOURCE.toString().toLowerCase())
          .put("role", Roles.PROVIDER.toString().toLowerCase());
      
      given().auth().oauth2(providerBravoToken)
      .contentType(ContentType.JSON).body(tokenReq.toString())
      .when().post("/token").then().statusCode(403)
      .body("type", equalTo(Urn.URN_INVALID_INPUT.toString()))
      .body("title", equalTo(ACCESS_DENIED))
    ...
    void resItemSuccess()
    {
      ...
      String token = given().auth().oauth2(providerAlphaToken)
      .contentType(ContentType.JSON).body(tokenReq.toString())
      .when().post("/token").then().statusCode(200)
      .body("type", equalTo(Urn.URN_SUCCESS.toString()))
      .body("title", equalTo(TOKEN_SUCCESS))
    ...
    void resServerTokenSuccess()
    {
      ...
      String token = given().auth().oauth2(providerBravoToken)
      .contentType(ContentType.JSON).body(tokenReq.toString())
      .when().post("/token").then().statusCode(200)
      .body("type", equalTo(Urn.URN_SUCCESS.toString()))
      .body("title", equalTo(TOKEN_SUCCESS))
  ...
  class AdminTokenTests
  {
    ...
    void noResGrpToken()
    {
      ...
      given().auth().oauth2(adminRealRsToken)
      .contentType(ContentType.JSON).body(tokenReq.toString())
      .when().post("/token").then().statusCode(403)
      .body("type", equalTo(Urn.URN_INVALID_INPUT.toString()))
      .body("title", equalTo(ERR_TITLE_NO_RES_GRP_TOKEN))
    ...
    void resServerNoRoleForRs()
    {
      ...
      given().auth().oauth2(adminDummyRsToken)
      .contentType(ContentType.JSON).body(tokenReq.toString())
      .when().post("/token").then().statusCode(403)
      .body("type", equalTo(Urn.URN_INVALID_INPUT.toString()))
      .body("title", equalTo(ACCESS_DENIED))
    ...
    void resServerTokenSuccess()
    {
      ...
      String token = given().auth().oauth2(adminRealRsToken)
      .contentType(ContentType.JSON).body(tokenReq.toString())
      .when().post("/token").then().statusCode(200)
      .body("type", equalTo(Urn.URN_SUCCESS.toString()))
      .body("title", equalTo(TOKEN_SUCCESS))
  ...
  class TrusteeTokenTests
  {
    ...
    void noResGrpToken()
    {
      ...
      given().auth().oauth2(trusteeToken)
      .contentType(ContentType.JSON).body(tokenReq.toString())
      .when().post("/token").then().statusCode(400)
      .body("type", equalTo(Urn.URN_INVALID_INPUT.toString()));
    }
  ...
  class CosAdminTokenTests
  {
    ...
    void noResGrpToken(KcAdminInt kc)
    {
      ...
      given().auth().oauth2(kc.cosAdminToken)
      .contentType(ContentType.JSON).body(tokenReq.toString())
      .when().post("/token").then().statusCode(403)
      .body("type", equalTo(Urn.URN_INVALID_INPUT.toString()))
      .body("title", equalTo(ERR_TITLE_NO_RES_GRP_TOKEN))
    ...
    {
      JsonObject tokenReq = new JsonObject().put("itemId", REAL_APD_URL)
          .put("itemType", ItemType.COS.toString().toLowerCase())
          .put("role", Roles.COS_ADMIN.toString().toLowerCase());
    ...
    void cosSuccess(KcAdminInt kc)
    {
      ...
      String token = given().auth().oauth2(kc.cosAdminToken)
      .contentType(ContentType.JSON).body(tokenReq.toString())
      .when().post("/token").then().statusCode(200)
      .body("type", equalTo(Urn.URN_SUCCESS.toString()))
      .body("title", equalTo(TOKEN_SUCCESS))
  ...
  class ConsumerDelegateTokenTests
  {
    ...
    void setup(KcAdminInt kc)
    {
      ...
      given().auth().oauth2(consumerDummyRsToken).contentType(ContentType.JSON)
      ...
      given().auth().oauth2(consumerRealRsToken).contentType(ContentType.JSON)
      .body(consRealRsReq.toString())
      .when().post("/delegations").then().statusCode(201)
      .body("type", equalTo(Urn.URN_SUCCESS.toString()));
      
      Response resp = given().auth().oauth2(delegateToken).contentType(ContentType.JSON)
      .when().get("/delegations").then().statusCode(200)
          .extract().response();
      
      delegationIdForConsumerDummyRs = resp.jsonPath()
          .param("url", DUMMY_RS_SERVER)
          .getString(
              "results.find { it.url == url }.id");
      delegationIdForConsumerRealRs = resp.jsonPath()
    ...
    void delegationIdMissing()
    {
      ...
      given().auth().oauth2(delegateToken)
      .contentType(ContentType.JSON).body(tokenReq.toString())
      .when().post("/token").then().statusCode(400)
      .body("type", equalTo(Urn.URN_MISSING_INFO.toString()))
    ...
    void noResGrpToken()
    {
      ...
      given().auth().oauth2(delegateToken).header("delegationId", delegationIdForConsumerRealRs)
      .contentType(ContentType.JSON).body(tokenReq.toString())
      .when().post("/token").then().statusCode(403)
      .body("type", equalTo(Urn.URN_INVALID_INPUT.toString()))
      .body("title", equalTo(ERR_TITLE_NO_RES_GRP_TOKEN))
    ...
    void resItemNoRoleForRs()
    {
      ...
      given().auth().oauth2(delegateToken).header("delegationId", delegationIdForConsumerDummyRs)
      .contentType(ContentType.JSON).body(tokenReq.toString())
      .when().post("/token").then().statusCode(403)
      .body("type", equalTo(Urn.URN_INVALID_INPUT.toString()))
      .body("title", equalTo(ACCESS_DENIED))
    ...
    void resItemApdSuccessNoConstraints()
    {
      ...
      String token = given().auth().oauth2(delegateToken).header("delegationId", delegationIdForConsumerRealRs)
      .contentType(ContentType.JSON).body(tokenReq.toString())
      .when().post("/token").then().statusCode(200)
      .body("type", equalTo(Urn.URN_SUCCESS.toString()))
      .body("title", equalTo(TOKEN_SUCCESS))
    ...
    @Test
    @DisplayName("Res Item token APD deny needs interaction + introspect - contains delegate in `sub`")
    void resItemApdDenyNeedsInteraction()
    {
      JsonObject tokenReq = new JsonObject().put("itemId", BRAVO_RES_ITEM_SECURE_ID)
          .put("itemType", ItemType.RESOURCE.toString().toLowerCase())
  ...
  class ProviderDelegateTokenTests
  {
    ...
    void setup(KcAdminInt kc)
    {
      ...
      given().auth().oauth2(providerDummyRsToken).contentType(ContentType.JSON)
      ...
      given().auth().oauth2(providerAlphaToken).contentType(ContentType.JSON)
      .body(alphaBravoReq.toString())
      .when().post("/delegations").then().statusCode(201)
      .body("type", equalTo(Urn.URN_SUCCESS.toString()));
      
      given().auth().oauth2(providerBravoToken).contentType(ContentType.JSON)
      .body(alphaBravoReq.toString())
      .when().post("/delegations").then().statusCode(201)
      .body("type", equalTo(Urn.URN_SUCCESS.toString()));
      ...
      delegationIdForProviderDummyRs = resp.jsonPath()
          .param("url", DUMMY_RS_SERVER)
          .getString(
              "results.find { it.url == url }.id");
      delegationIdForProviderAlpha = resp.jsonPath()
          .param("url", REAL_RS_URL).param("email", PROVIDER_ALPHA_EMAIL)
          .getString(
              "results.find { it.url == url && it.owner.email == email }.id");
      delegationIdForProviderBravo = resp.jsonPath()
          .param("url", REAL_RS_URL).param("email", PROVIDER_BRAVO_EMAIL)
          .getString(
              "results.find { it.url == url && it.owner.email == email }.id");
    }
    ...
    void noResGrpToken()
    {
      ...
      given().auth().oauth2(delegateToken).header("delegationId", delegationIdForProviderAlpha)
      .contentType(ContentType.JSON).body(tokenReq.toString())
      .when().post("/token").then().statusCode(403)
      .body("type", equalTo(Urn.URN_INVALID_INPUT.toString()))
      .body("title", equalTo(ERR_TITLE_NO_RES_GRP_TOKEN))
    ...
    void resItemNoRoleForRs()
    {
      ...
      // delegation for DUMMY_SERVER will not work since request is for res on REAL_RS
      given().auth().oauth2(delegateToken).header("delegationId", delegationIdForProviderDummyRs)
      .contentType(ContentType.JSON).body(tokenReq.toString())
      .when().post("/token").then().statusCode(403)
      .body("type", equalTo(Urn.URN_INVALID_INPUT.toString()))
      .body("title", equalTo(ACCESS_DENIED))
    ...
    void resItemNotOwnResource()
    {
      ...
      // Delegation for Alpha provider will not work - Alpha provider does not own resource, belongs to Bravo provider 
      given().auth().oauth2(delegateToken).header("delegationId", delegationIdForProviderAlpha)
      .contentType(ContentType.JSON).body(tokenReq.toString())
      .when().post("/token").then().statusCode(403)
      .body("type", equalTo(Urn.URN_INVALID_INPUT.toString()))
    ...
    void resItemNoAccessToPiiResource()
    {
      ...
      given().auth().oauth2(delegateToken).header("delegationId", delegationIdForProviderBravo)
      .contentType(ContentType.JSON).body(tokenReq.toString())
      .when().post("/token").then().statusCode(403)
      .body("type", equalTo(Urn.URN_INVALID_INPUT.toString()))
      .body("title", equalTo(ACCESS_DENIED))
    ...
    void resItemSuccess()
    {
      ...
      String token = given().auth().oauth2(delegateToken).header("delegationId", delegationIdForProviderAlpha)
      .contentType(ContentType.JSON).body(tokenReq.toString())
      .when().post("/token").then().statusCode(200)
      .body("type", equalTo(Urn.URN_SUCCESS.toString()))
      .body("title", equalTo(TOKEN_SUCCESS))
    ...
    void resServerTokenSuccess()
    {
      ...
      String token = given().auth().oauth2(delegateToken).header("delegationId", delegationIdForProviderBravo)
      .contentType(ContentType.JSON).body(tokenReq.toString())
      .when().post("/token").then().statusCode(200)
      .body("type", equalTo(Urn.URN_SUCCESS.toString()))
      .body("title", equalTo(TOKEN_SUCCESS))
