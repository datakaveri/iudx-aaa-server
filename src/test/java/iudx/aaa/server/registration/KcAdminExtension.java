 ...
 * of a {@link KcAdminInt} object globally (in the JUnit root {@link ExtensionContext}). This allows
 * the {@link KcAdminInt} object to be instantiated only once, before any integration test starts.
 *
 * <p></br> </br> Additionally {@link KcAdminInt} implements {@link CloseableResource} so that at
 * the end of all tests, Keycloak users can be cleaned up automatically.
 */
public class KcAdminExtension implements BeforeAllCallback, ParameterResolver {
  ...
  public void beforeAll(ExtensionContext context) {
    if (context.getRoot().getStore(NAMESPACE).get(KC_EXT, KcAdminInt.class) == null) {
      try {
        ...
        LOGGER.info(
            "Created KcAdminInt object and placed in the JUnit root ExtensionContext store");
      } catch (IOException e) {
  ...
  @Override
  public KcAdminInt resolveParameter(ParameterContext parameterContext, ExtensionContext context) {
