...
/**
 * Testing get delegate emails and search user - both can be only performed by trustee. Client ID
 * and client secret validation tests are done in {@link ClientCredentialsIT}.
 *
 */
...
public class TrusteeOperationsIT {
  ...
  void searchUserUserMissingOneOfIdOrSecret(KcAdminInt kc) {
    ...
    given().when().headers("clientId", trusteeClientCreds.get("clientId"))
    .queryParams(queryParams)
    .get("/user/search").then().statusCode(400)
    .body("type", equalTo(Urn.URN_INVALID_INPUT.toString()));
    
    given().when().headers("clientSecret", trusteeClientCreds.get("clientSecret"))
    .queryParams(queryParams)
    .get("/user/search").then().statusCode(400)
    .body("type", equalTo(Urn.URN_INVALID_INPUT.toString()));
  }
