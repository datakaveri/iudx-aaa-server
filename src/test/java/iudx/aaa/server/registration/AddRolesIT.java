...
public class AddRolesIT {
  ...
  void missingBody() {
    ...
    given().auth().oauth2(tokenForInvalidTests).contentType(ContentType.JSON).when()
    .post("/user/roles").then()
    .statusCode(400)
  ...
  void missingAndInvalidKeys() {
    ...
    given().auth().oauth2(tokenForInvalidTests).contentType(ContentType.JSON).body(req.toString()).when()
    .post("/user/roles").then()
    .statusCode(400)
    .body("type", equalTo(Urn.URN_INVALID_INPUT.toString()));
    ...
    req = new JsonObject()
        .put(Roles.CONSUMER.toString().toLowerCase(), new JsonArray(List.of(ALPHA_SERVER)))
        .put(Roles.PROVIDER.toString().toLowerCase(), new JsonArray(List.of(ALPHA_SERVER)))
        .put("phone", "9889889881").put("userInfo", RandomStringUtils.random(10));
    ...
    req = new JsonObject()
        .put(Roles.CONSUMER.toString().toLowerCase(), new JsonArray(List.of(ALPHA_SERVER)))
        .put(Roles.PROVIDER.toString().toLowerCase(), new JsonArray(List.of(ALPHA_SERVER)))
        .put("phone", "9889889881").put("userInfo", new JsonObject().put("1", true).put("2", true)
            .put("3", true).put("4", true).put("5", true).put("6", true));
  }
  ...
  void invalidConsProvArrays() {
    ...
    req = new JsonObject()
        .put(Roles.CONSUMER.toString().toLowerCase(), new JsonArray(List.of(ALPHA_SERVER, "%%7381498.com")))
        .put(Roles.PROVIDER.toString().toLowerCase(), new JsonArray(List.of(ALPHA_SERVER)));
    ...
    req = new JsonObject()
        .put(Roles.CONSUMER.toString().toLowerCase(), new JsonArray(List.of(ALPHA_SERVER)))
        .put(Roles.PROVIDER.toString().toLowerCase(), new JsonArray(List.of(ALPHA_SERVER, "%%7381498.com")));
    ...
    req = new JsonObject()
        .put(Roles.CONSUMER.toString().toLowerCase(), new JsonArray(List.of(ALPHA_SERVER, ALPHA_SERVER)))
        .put(Roles.PROVIDER.toString().toLowerCase(), new JsonArray(List.of(ALPHA_SERVER)));
    ...
    req = new JsonObject()
        .put(Roles.CONSUMER.toString().toLowerCase(), new JsonArray(List.of(ALPHA_SERVER)))
        .put(Roles.PROVIDER.toString().toLowerCase(), new JsonArray(List.of(ALPHA_SERVER, ALPHA_SERVER)));
    ...
    req = new JsonObject()
        .put(Roles.CONSUMER.toString().toLowerCase(), new JsonArray(List.of(ALPHA_SERVER.toUpperCase())))
        .put(Roles.PROVIDER.toString().toLowerCase(), new JsonArray(List.of(ALPHA_SERVER)));
    ...
    req = new JsonObject()
        .put(Roles.CONSUMER.toString().toLowerCase(), new JsonArray(List.of(ALPHA_SERVER)))
        .put(Roles.PROVIDER.toString().toLowerCase(), new JsonArray(List.of(ALPHA_SERVER.toUpperCase())));
  ...
  void nonExistentRs() {
    ...
    JsonObject req = new JsonObject()
        .put(Roles.CONSUMER.toString().toLowerCase(), new JsonArray(List.of(ALPHA_SERVER, nonExistentServer1)))
        .put(Roles.PROVIDER.toString().toLowerCase(), new JsonArray(List.of(ALPHA_SERVER)));
    ...
    req = new JsonObject()
        .put(Roles.CONSUMER.toString().toLowerCase(), new JsonArray(List.of(ALPHA_SERVER)))
        .put(Roles.PROVIDER.toString().toLowerCase(), new JsonArray(List.of(ALPHA_SERVER, nonExistentServer1)));
    ...
    req = new JsonObject()
        .put(Roles.CONSUMER.toString().toLowerCase(), new JsonArray(List.of(ALPHA_SERVER, nonExistentServer1)))
        .put(Roles.PROVIDER.toString().toLowerCase(), new JsonArray(List.of(ALPHA_SERVER, nonExistentServer2)));
    ...
    .body("title", equalTo(ERR_TITLE_RS_NO_EXIST))
    .body("detail", equalTo(ERR_DETAIL_RS_NO_EXIST))
    .body("context." + ERR_CONTEXT_NOT_FOUND_RS_URLS , containsInAnyOrder(nonExistentServer1, nonExistentServer2));
  }
  ...
  class AddingConsumerRoleTests {
    ...
    void aliceConsumerSingleRsSuccess() {
      JsonObject req = new JsonObject()
          .put(Roles.CONSUMER.toString().toLowerCase(), new JsonArray(List.of(ALPHA_SERVER)))
          .put("phone", alicePhone).put("userInfo", aliceUserInfo);

      given().auth().oauth2(aliceToken).contentType(ContentType.JSON).body(req.toString()).when()
      .post("/user/roles").then()
      .statusCode(200)
    ...
    @Test
    @Order(4)
    @DisplayName("Register for provider for Alpha, Bravo and consumer for both servers - "
        + "fails because already has consumer roles for Alpha")
    void regProviderAndConsumerFails() {
      
      JsonObject req = new JsonObject()
          .put(Roles.CONSUMER.toString().toLowerCase(), new JsonArray(List.of(ALPHA_SERVER, BRAVO_SERVER)))
          .put(Roles.PROVIDER.toString().toLowerCase(), new JsonArray(List.of(ALPHA_SERVER, BRAVO_SERVER)));
    ...
    @Test
    @Order(5)
    @DisplayName("Alice register for provider roles for Alpha and Bravo and consumer role for Bravo")
    ...
    @Test
    @Order(6)
    @DisplayName("Listing roles for Alice - has consumer role for Alpha, Bravo - no provider since is pending")
    ...
    void adminCanSeePendingReg() {
      ...
      .body("results.find { it.email == '%s' && it.rsUrl == '%s' }.userInfo",
  ...
  class AddingProviderRoleTests {
    ...
    void bobProviderSingleRsSuccess() {
      JsonObject req = new JsonObject()
          .put(Roles.PROVIDER.toString().toLowerCase(), new JsonArray(List.of(ALPHA_SERVER)))
          .put("phone", bobPhone).put("userInfo", bobUserInfo);

      given().auth().oauth2(bobToken).contentType(ContentType.JSON).body(req.toString()).when()
      .post("/user/roles").then()
      .statusCode(200)
      .body("type", equalTo(Urn.URN_SUCCESS.toString()))
      .body("title", equalTo(SUCC_TITLE_ADDED_ROLES + PROVIDER_PENDING_MESG))
    ...
    void failRegAgain() {
      ...
      given().auth().oauth2(bobToken).contentType(ContentType.JSON).body(req.toString()).when()
      .post("/user/roles").then().statusCode(403)
      .body("type", equalTo(Urn.URN_INVALID_INPUT.toString()))
    ...
    @Test
    @Order(6)
    @DisplayName("Bob register for consumer roles for Alpha and Bravo and provider for Bravo "
    ...
    @Test
    @Order(7)
    @DisplayName("RS admin can see pending registration for Alpha, Bravo servers for Bob - userInfo unchanged")
    void adminCanSeePendingReg() {
    ...
    @Test
    @Order(8)
    @DisplayName("Listing roles for Bob - has consumer role for Alpha and Bravo - no provider since is pending state")
  ...
  @TestInstance(Lifecycle.PER_CLASS)
  @TestMethodOrder(OrderAnnotation.class)
  @DisplayName("Adding roles to rejected and approved providers - but phone and userinfo cannot be updated")
  class AddingRoleToApprovedRejectedProvidersTests {
    ...
    given().auth().oauth2(rejectedAlphaProvToken).contentType(ContentType.JSON).body(roleReq.toString()).when()
        .post("/user/roles").then()
        .statusCode(describedAs("Setup - Register provider (to be rejected) "
            + "for Alpha RS Server - no phone, no userinfo", is(200)));
    
    given().auth().oauth2(approvedAlphaProvToken).contentType(ContentType.JSON).body(roleReq.toString()).when()
        .post("/user/roles").then()
        .statusCode(describedAs("Setup - Register provider (to be approved) "
            + "for Alpha RS Server - no phone, no userinfo", is(200)));
    ...
    String rejectingAlphaId =
        response.jsonPath().param("email", rejectedAlphaProvEmail).param("rsUrl", ALPHA_SERVER)
            .getString("results.find { it.email == email && it.rsUrl == rsUrl}.id");
    String approvingAlphaId =
        response.jsonPath().param("email", approvedAlphaProvEmail).param("rsUrl", ALPHA_SERVER)
            .getString("results.find { it.email == email && it.rsUrl == rsUrl}.id");
    
    JsonObject approveAndRejectReq = new JsonObject().put("request", new JsonArray().add(new JsonObject()
        .put("status", RoleStatus.REJECTED.toString().toLowerCase()).put("id", rejectingAlphaId)).add(new JsonObject()
        .put("status", RoleStatus.APPROVED.toString().toLowerCase()).put("id", approvingAlphaId))
        );
    
    given().auth().oauth2(tokenRoles).contentType(ContentType.JSON).body(approveAndRejectReq.toString()).when()
        .put("/admin/provider/registrations").then()
        .statusCode(describedAs("Setup - Approve and Reject providers on Alpha RS server", is(200)));
    }
    
    @Test
    @Order(1)
    @DisplayName("Rejected provider on Alpha cannot register for all servers as provider and consumer, since rejected on Alpha")
    void rejProviderFailAllRolesAllServers() {
      JsonObject req = new JsonObject()
          .put(Roles.PROVIDER.toString().toLowerCase(), new JsonArray(List.of(ALPHA_SERVER, BRAVO_SERVER)))
          .put(Roles.CONSUMER.toString().toLowerCase(), new JsonArray(List.of(ALPHA_SERVER, BRAVO_SERVER)));

      given().auth().oauth2(rejectedAlphaProvToken).contentType(ContentType.JSON).body(req.toString()).when()
      .post("/user/roles").then()
      .statusCode(403)
      .body("type", equalTo(Urn.URN_INVALID_INPUT.toString()))
      .body("title", equalTo(ERR_TITLE_PENDING_REJECTED_PROVIDER_RS_REG_EXISTS))
    ...
    @Test
    @Order(2)
    @DisplayName("Rejected provider successfully registers for provider on Bravo server and consumer for both servers"
        + " - cannot update phone")
    void rejProviderSuccess() {
      ...
      .body("type", equalTo(Urn.URN_SUCCESS.toString()))
      .body("title", equalTo(SUCC_TITLE_ADDED_ROLES + PROVIDER_PENDING_MESG))
      .body("results.userId", equalTo(IntegTestHelpers.getUserIdFromKcToken(rejectedAlphaProvToken)))
      .body("results.roles", containsInAnyOrder(Roles.CONSUMER.toString().toLowerCase()))
      .body("results.rolesToRsMapping.consumer", containsInAnyOrder(ALPHA_SERVER, BRAVO_SERVER))
      .body("results.rolesToRsMapping.provider", is(nullValue()))
    ...
    void listRolesRejProvider() {
      ...
      given().auth().oauth2(rejectedAlphaProvToken).when()
      .get("/user/roles").then()
      .statusCode(200)
      .body("type", equalTo(Urn.URN_SUCCESS.toString()))
      .body("title", equalTo(SUCC_TITLE_USER_READ))
    ...
    @Test
    @Order(5)
    @DisplayName("Approved provider on Alpha cannot register as provider for all servers, since approved on Alpha")
    void approvedProviderAllServerFails() {
      JsonObject req = new JsonObject()
          .put(Roles.PROVIDER.toString().toLowerCase(), new JsonArray(List.of(ALPHA_SERVER, BRAVO_SERVER)));

      given().auth().oauth2(approvedAlphaProvToken).contentType(ContentType.JSON).body(req.toString()).when()
      .post("/user/roles").then()
      .statusCode(409)
      .body("type", equalTo(Urn.URN_ALREADY_EXISTS.toString()))
      .body("title", equalTo(ERR_TITLE_ROLE_FOR_RS_EXISTS))
    ...
    @Test
    @Order(6)
    @DisplayName("Approved provider successfully registers for provider on Bravo server and consumer for both servers"
        + " - cannot update phone, try updating userinfo from empty")
    void approvedProviderSuccess() {
      JsonObject req = new JsonObject()
          .put(Roles.PROVIDER.toString().toLowerCase(), new JsonArray(List.of(BRAVO_SERVER)))
          .put(Roles.CONSUMER.toString().toLowerCase(), new JsonArray(List.of(ALPHA_SERVER, BRAVO_SERVER)))
          .put("phone", "8998998992").put("userInfo", new JsonObject().put("approved", true));
      ...
      .body("type", equalTo(Urn.URN_SUCCESS.toString()))
      .body("title", equalTo(SUCC_TITLE_ADDED_ROLES + PROVIDER_PENDING_MESG))
      .body("results.userId", equalTo(IntegTestHelpers.getUserIdFromKcToken(approvedAlphaProvToken)))
      .body("results.roles", containsInAnyOrder(Roles.CONSUMER.toString().toLowerCase(), Roles.PROVIDER.toString().toLowerCase()))
      .body("results.rolesToRsMapping.consumer", containsInAnyOrder(ALPHA_SERVER, BRAVO_SERVER))
    ...
    @Test
    @Order(7)
    @DisplayName("Listing roles for approved provider - has consumer role for Alpha and Bravo - and provider role for Alpha")
    void listRolesApprovedProvider() {
      
      given().auth().oauth2(approvedAlphaProvToken).when()
      .get("/user/roles").then()
      .statusCode(200)
      .body("type", equalTo(Urn.URN_SUCCESS.toString()))
      .body("title", equalTo(SUCC_TITLE_USER_READ))
    ...
    @Test
    @Order(8)
    @DisplayName("RS admin can see pending registration for Bravo for approved provider - userinfo empty")
  ...
  class PendingProviderScenarios
  {
    ...
    void trusteeSearchFail() {
      ...
      given().when().headers(tokenRolesClientCreds)
  ...
  @TestInstance(Lifecycle.PER_CLASS)
  @TestMethodOrder(OrderAnnotation.class)
  @DisplayName("Users having other roles (admin, trustee) adding consumer/provider roles - but phone or userinfo cannot be updated")
  class OtherRolesTests {
  
    @Test
    @Order(1)
    @DisplayName("Add consumer and provider role for Alpha, Bravo servers with phone for tokenRoles user "
    ...
    void tokenRolesAddingRolesSuccess() {
      ...
      JsonObject req = new JsonObject()
          .put(Roles.CONSUMER.toString().toLowerCase(), new JsonArray(List.of(ALPHA_SERVER, BRAVO_SERVER)))
          .put(Roles.PROVIDER.toString().toLowerCase(), new JsonArray(List.of(ALPHA_SERVER, BRAVO_SERVER)))
          .put("phone", "9009009001").put("userInfo", new JsonObject().put("otherRoles", true));
      ...
      .body("results.roles",
          containsInAnyOrder(Roles.CONSUMER.toString().toLowerCase(),
              Roles.ADMIN.toString().toLowerCase(), Roles.TRUSTEE.toString().toLowerCase()))
      .body("results.rolesToRsMapping.consumer", containsInAnyOrder(ALPHA_SERVER, BRAVO_SERVER))
      ...
      .body("title", equalTo(ERR_TITLE_ROLE_FOR_RS_EXISTS))
      .body("detail", equalTo(ERR_DETAIL_CONSUMER_FOR_RS_EXISTS))
      .body("context." + ERR_CONTEXT_EXISTING_ROLE_FOR_RS, containsInAnyOrder(ALPHA_SERVER, BRAVO_SERVER));
    }  
    ...
    @Test
    @Order(3)
    @DisplayName("RS admin can see pending registration for Alpha and Bravo for themself - userinfo not updated")
