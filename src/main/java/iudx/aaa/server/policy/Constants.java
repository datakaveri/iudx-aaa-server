...
"UPDATE apd_policies SET status = 'DELETED', updated_at = NOW() WHERE id = ANY($1::uuid[])";
  ...
  public static final String GET_PROVIDER_AUTH_DELEGATE =
      "WITH auth AS (SELECT owner_id, id FROM resource_server WHERE url = $1::text )"
      + ", delegate AS (SELECT resource_server_id, user_id, owner_id as provider_id FROM delegations"
      + " WHERE owner_id = ANY($2::uuid[]) AND status = 'ACTIVE' AND resource_server_id = (SELECT id FROM auth))"
      + ", policy AS (SELECT user_id FROM policies WHERE item_id = (SELECT id FROM auth)"
      + " AND owner_id = (SELECT owner_id FROM auth) AND user_id = ANY((SELECT user_id FROM delegate))"
      + " AND status = 'ACTIVE' AND expiry_time > NOW())"
