...
import iudx.aaa.server.registration.ClientCredentialsIT;
...
/**
 * Integration tests for testing get delegate emails and search user - both can be only performed by
 * trustee. Client ID and client secret validation tests are done in {@link ClientCredentialsIT}.
 */
...
public class TrusteeOperationsIT {
  ...
  private static String DUMMY_SERVER =
  ...
  static void setup(KcAdminInt kc) {
    ...
    String trusteeClientId =
        new JsonObject(clientInfoTrustee).getJsonObject("results").getString(RESP_CLIENT_ID);
    String trusteeClientSecret =
        new JsonObject(clientInfoTrustee).getJsonObject("results").getString(RESP_CLIENT_SC);
  ...
  void searchUserMissingAndExtraParams() {
    Map<String, String> queryParams =
        Map.of(
            "email",
            RandomStringUtils.random(10) + "@gmail.com",
            "resourceServer",
            DUMMY_SERVER,
            "role",
            Roles.CONSUMER.toString().toLowerCase(),
    ...
    queryParams =
        Map.of(
            "email", RandomStringUtils.random(10) + "@gmail.com", "resourceServer", DUMMY_SERVER);
    ...
    queryParams = Map.of("userId", UUID.randomUUID().toString(), "resourceServer", DUMMY_SERVER);
  ...
  @Test
  @DisplayName("Search user - missing one of client ID or secret")
  void searchUserUserMissingOneOfIdOrSecret(KcAdminInt kc) {
    Map<String, String> queryParams =
        Map.of(
            "email",
            noRolesEmail,
    ...
    given()
        .when()
        .headers("clientId", trusteeClientCreds.get("clientId"))
        .queryParams(queryParams)
        .get("/user/search")
        .then()
        .statusCode(400)
    ...
    given()
        .when()
        .headers("clientSecret", trusteeClientCreds.get("clientSecret"))
        .queryParams(queryParams)
        .get("/user/search")
        .then()
        .statusCode(400)
  ...
  class SearchUserCases {
    ...
    void setup(KcAdminInt kc) {
      ...
      consumerUserId =
          given()
              ...
              .post("/user/roles")
              .then()
              .statusCode(describedAs("Added consumer", is(200)))
              .extract()
              .path("results.userId");
      ...
      providerUserId =
          given()
              ...
              .post("/user/roles")
              .then()
              .statusCode(describedAs("Added provider", is(200)))
              .extract()
              .path("results.userId");
      ...
      String providerRegId =
          given()
              ...
              .get("/admin/provider/registrations")
              .then()
              .statusCode(describedAs("Get provider reg. ID", is(200)))
              .extract()
    ...
    void rsDoesNotExist(KcAdminInt kc) {
      ...
      Map<String, String> queryParamsId =
          Map.of(
              "userId",
              consumerUserId,
              "resourceServer",
  ...
  @Test
  @DisplayName("Get delegate emails - missing client ID or secret")
  void getDelegateEmailsMissingIdOrSecret(KcAdminInt kc) {
    Map<String, String> queryParams =
        Map.of(
            "userId",
            noRolesUserId,
  ...
  class GetDelegateEmailsCases {
    ...
    void createDelegs(KcAdminInt kc) {
      ...
      JsonObject consumerDelegateReq =
          new JsonObject()
              .put(
                  "request",
                  new JsonArray()
                      .add(
                          new JsonObject()
                              .put("userEmail", delegateAlphaEmail)
                              .put("resSerUrl", DUMMY_SERVER)
                              .put("role", Roles.CONSUMER.toString().toLowerCase()))
                      .add(
                          new JsonObject()
                              .put("userEmail", delegateBravoEmail)
                              .put("resSerUrl", DUMMY_SERVER)
                              .put("role", Roles.CONSUMER.toString().toLowerCase())));
      ...
      given()
          ...
          .oauth2(consumerToken)
          .contentType(ContentType.JSON)
          .body(consumerDelegateReq.toString())
          .when()
          .post("/delegations")
          .then()
      ...
      JsonObject providerDelegateReq =
          new JsonObject()
              .put(
                  "request",
                  new JsonArray()
      ...
      given()
          ...
          .oauth2(providerToken)
          .contentType(ContentType.JSON)
          .body(providerDelegateReq.toString())
          .when()
          .post("/delegations")
          .then()
    ...
    @Test
    @DisplayName(
        "Getting deleg emails - get 2 consumer delegates for DUMMY_SERVER - non-empty response")
    ...
    @Test
    @DisplayName(
        "Getting deleg emails - get 1 provider delegate for DUMMY_SERVER - non-empty response")
