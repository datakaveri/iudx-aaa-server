...
public class GetAndUpdateProviderRegistrationIT {
  ...
  private static String CHARLIE_SERVER = RandomStringUtils.randomAlphabetic(10).toLowerCase() + ".com";
  ...
  static void setup(KcAdminInt kc) {
    ...
    JsonObject rsReqBravo =
        new JsonObject().put("name", "name").put("url", BRAVO_SERVER).put("owner", adminBravoCharlie);
    ...
    JsonObject rsReqCharlie =
        new JsonObject().put("name", "name").put("url", CHARLIE_SERVER).put("owner", adminBravoCharlie);

    given().auth().oauth2(kc.cosAdminToken).contentType(ContentType.JSON).body(rsReqCharlie.toString())
        .when().post("/admin/resourceservers").then()
        .statusCode(describedAs("Setup - Created Charlie RS server", is(201)));
    
    // create consumer, provider roles for Alpha server only
    JsonObject consReq = new JsonObject().put("consumer", new JsonArray().add(ALPHA_SERVER))
    ...
    given().auth().oauth2(tokenRoles).contentType(ContentType.JSON).body(consReq.toString()).when()
        ...
        .statusCode(describedAs("Setup - Added consumer, provider roles for Alpha RS Server", is(200)));
    
    String providerRegId =  
    given().auth().oauth2(tokenAdminAlpha).contentType(ContentType.JSON).when()
    ...
    given().auth().oauth2(tokenAdminAlpha).contentType(ContentType.JSON).body(approveReq.toString()).when()
    ...
    JsonObject delegatorConsReq = new JsonObject().put("consumer", new JsonArray().add(ALPHA_SERVER));
  ...
  void getProvNoToken()
  {
    given().when().get("/admin/provider/registrations").then().statusCode(401).body("type",
        equalTo(Urn.URN_MISSING_AUTH_TOKEN.toString()));
  }
  
  @Test
  @DisplayName("Get Provider Reg - Consumer, provider, delegate, trustee, COS admin cannot call API")
  void getProvRolesCannotCall(KcAdminInt kc)
  {
    given().auth().oauth2(tokenRoles).when().get("/admin/provider/registrations").then()
    .statusCode(401)
    ...
    given().auth().oauth2(kc.cosAdminToken).when().get("/admin/provider/registrations").then()
    .statusCode(401)
    .body("type", equalTo(Urn.URN_INVALID_ROLE.toString()))
    .body("title", equalTo(ERR_TITLE_NOT_ADMIN))
    .body("detail", equalTo(ERR_DETAIL_NOT_ADMIN));
  ...
  {
    given().auth().oauth2(tokenNoRoles).when().get("/admin/provider/registrations").then()
    .statusCode(401)
    .body("type", equalTo(Urn.URN_INVALID_ROLE.toString()))
    .body("title", equalTo(ERR_TITLE_NOT_ADMIN))
    .body("detail", equalTo(ERR_DETAIL_NOT_ADMIN));
  ...
  {
    given().auth().oauth2(tokenAdminAlpha).queryParam("filter", "sdawnejdjandqw").when()
    .get("/admin/provider/registrations").then()
    .statusCode(400)
  ...
  {
    JsonObject request = new JsonObject().put("request",
        new JsonArray().add(new JsonObject().put("id", UUID.randomUUID().toString()).put("status",
            RoleStatus.APPROVED.toString().toLowerCase()))
        );
    
    given().when().body(request.toString()).put("/admin/provider/registrations").then()
        .statusCode(401).body("type", equalTo(Urn.URN_MISSING_AUTH_TOKEN.toString()));
  }
  ...
  void updateProvRolesCannotCall(KcAdminInt kc)
  {
    ...
    given().auth().oauth2(tokenRoles).contentType(ContentType.JSON).body(request.toString())
    .when().put("/admin/provider/registrations").then()
    .statusCode(401)
    .body("type", equalTo(Urn.URN_INVALID_ROLE.toString()))
    .body("title", equalTo(ERR_TITLE_NOT_ADMIN))
    .body("detail", equalTo(ERR_DETAIL_NOT_ADMIN));
    
    given().auth().oauth2(kc.cosAdminToken).contentType(ContentType.JSON).body(request.toString())
    .when().put("/admin/provider/registrations").then()
    .statusCode(401)
    .body("type", equalTo(Urn.URN_INVALID_ROLE.toString()))
    .body("title", equalTo(ERR_TITLE_NOT_ADMIN))
  ...
  void updateProvNoRolesCannotCall()
  {
    ...
    given().auth().oauth2(tokenNoRoles).contentType(ContentType.JSON).body(request.toString())
    .when().put("/admin/provider/registrations").then()
    .statusCode(401)
    .body("type", equalTo(Urn.URN_INVALID_ROLE.toString()))
    .body("title", equalTo(ERR_TITLE_NOT_ADMIN))
  ...
  {
    given().auth().oauth2(tokenAdminAlpha).body(new JsonObject().toString()).when().put("/admin/provider/registrations").then()
    .statusCode(400)
    .body("type", equalTo(Urn.URN_INVALID_INPUT.toString()));
  }
  ...
  void updateProvInvalidBody() {
    ...
    given().auth().oauth2(tokenAdminAlpha).contentType(ContentType.JSON).body(request.toString()).when().put("/admin/provider/registrations")
        .then().statusCode(400).body("type", equalTo(Urn.URN_INVALID_INPUT.toString()));

    request = new JsonObject().put("request", new JsonArray()
        .add(new JsonObject().put("status", RoleStatus.APPROVED.toString().toLowerCase())));

    given().auth().oauth2(tokenAdminAlpha).contentType(ContentType.JSON).body(request.toString()).when().put("/admin/provider/registrations")
        .then().statusCode(400).body("type", equalTo(Urn.URN_INVALID_INPUT.toString()));

    request = new JsonObject().put("id", UUID.randomUUID().toString()).put("status",
        RoleStatus.APPROVED.toString().toLowerCase());

    given().auth().oauth2(tokenAdminAlpha).contentType(ContentType.JSON).body(request.toString()).when().put("/admin/provider/registrations")
        .then().statusCode(400).body("type", equalTo(Urn.URN_INVALID_INPUT.toString()));

    request = new JsonObject().put("request",
        new JsonArray().add(new JsonObject().put("id", RandomStringUtils.random(10)).put("status",
            RoleStatus.APPROVED.toString().toLowerCase())));
    ...
    request = new JsonObject().put("request", new JsonArray()
        .add(new JsonObject().put("id", UUID.randomUUID().toString()).put("status", "1234")));
  ...
  void updateProvRolesDuplicateIdsWithStatuses()
  {
    ...
    JsonObject request = new JsonObject().put("request",
        new JsonArray().add(new JsonObject().put("id", UUID.randomUUID().toString()).put("status",
            RoleStatus.APPROVED.toString().toLowerCase()))
        .add(new JsonObject().put("id", badId ).put("status",
            RoleStatus.REJECTED.toString().toLowerCase()))
        .add(new JsonObject().put("id", badId ).put("status",
            RoleStatus.APPROVED.toString().toLowerCase()))
        );
  ...
  class GetAndUpdateProviderRegTestTogether{
    ...
    void setup(KcAdminInt kc) {
      ...
      JsonObject provReq = new JsonObject()
          .put("provider", new JsonArray(List.of(ALPHA_SERVER, BRAVO_SERVER, CHARLIE_SERVER)));

      given().auth().oauth2(aliceToken).contentType(ContentType.JSON).body(provReq.toString()).when()
      .post("/user/roles").then()
      .statusCode(describedAs("Setup - Added provider reg for providerAlice for all RS Server", is(200)));

      given().auth().oauth2(bobToken).contentType(ContentType.JSON).body(provReq.toString()).when()
      .post("/user/roles").then()
      .statusCode(describedAs("Setup - Added provider reg for providerBob for all RS Server", is(200)));
    }
    ...
    void pendingProvidersHaveNoRoles() {
      ...
      given().auth().oauth2(aliceToken).when()
      .get("/user/roles").then()
      .statusCode(404)
      .body("results", is(nullValue()));
      
      given().auth().oauth2(bobToken).when()
      .get("/user/roles").then()
      .statusCode(404)
      .body("results", is(nullValue()));
    }
    ...
    @Test
    @Order(2)
    @DisplayName("Checking pending for Alpha admin and Bravo+Charlie admin - will have entries for Alice and Bob providers")
    void getPending()
    {
      given().auth().oauth2(tokenAdminAlpha).when()
      ...
      given().auth().oauth2(tokenAdminBravoCharlie).when()
      .queryParam("filter", RoleStatus.PENDING.toString().toLowerCase())
      .get("/admin/provider/registrations").then()
      .statusCode(200)
      .body("type", equalTo(Urn.URN_SUCCESS.toString()))
      ...
      .body("results.find { it.email == '%s' && it.rsUrl == '%s'}",
    ...
    @Test
    @Order(3)
    @DisplayName("Checking approved for Alpha admin and Bravo+Charlie admin - will have NO entries for Alice and Bob providers")
    void getApproved()
    {
      given().auth().oauth2(tokenAdminAlpha).when()
    ...
    @Test
    @Order(4)
    @DisplayName("Checking rejected for Alpha admin and Bravo+Charlie admin - will have NO entries for Alice and Bob providers")
    void getRejected()
    {
      given().auth().oauth2(tokenAdminAlpha).when()
    ...
    @Test
    @Order(5)
    @DisplayName("Bravo+Charlie admin tries to approve Alice for Alpha server - fails, does not own ID")
    void adminDoesNotOwnProviderRegId()
    {
      String aliceAlphaRegId = given().auth().oauth2(tokenAdminAlpha).when()
          .queryParam("filter", RoleStatus.PENDING.toString().toLowerCase())
          .get("/admin/provider/registrations").then()
          .statusCode(200)
          .extract()
      ...
      JsonObject request = new JsonObject().put("request",
          new JsonArray().add(new JsonObject().put("id", aliceAlphaRegId).put("status",
              RoleStatus.APPROVED.toString().toLowerCase()))
          );

        given().auth().oauth2(tokenAdminBravoCharlie).contentType(ContentType.JSON)
            .body(request.toString()).when().put("/admin/provider/registrations")
            .then().statusCode(400)
        .body("type", equalTo(Urn.URN_INVALID_INPUT.toString()))
    ...
    void approveSingleSuccessRepeatFails()
    {
      ...
      given().auth().oauth2(tokenAdminAlpha).contentType(ContentType.JSON)
        .body(request.toString()).when().put("/admin/provider/registrations")
        .then().statusCode(200)
        .body("type", equalTo(Urn.URN_SUCCESS.toString()))
    ...
    @Test
    @Order(7)
    @DisplayName("Alpha admin checks pending, rejected and approved - Alice in approved, Bob still pending")
    ...
    @Test
    @Order(8)
    @DisplayName("Alpha provider tries reject Bob w/ invalid ID in request - fails - Bob is not rejected")
    void rejectedMultipleInvalidId()
    {
      
      String bobAlphaRegId = given().auth().oauth2(tokenAdminAlpha).when()
          .queryParam("filter", RoleStatus.PENDING.toString().toLowerCase())
          .get("/admin/provider/registrations").then()
          .statusCode(200)
          .extract()
      ...
      JsonObject request = new JsonObject().put("request",
          new JsonArray().add(new JsonObject().put("id", bobAlphaRegId).put("status",
              RoleStatus.REJECTED.toString().toLowerCase()))
          .add(new JsonObject().put("id", randId).put("status",
              RoleStatus.REJECTED.toString().toLowerCase()))
          );
    ...
    @Test
    @Order(9)
    @DisplayName("Bravo+Charlie approves Alice, Bob for Bravo, rejects Alice, Bob for Charlie - repeat fails")
    void rejectApproveMultipleRequestsMultipleServers()
    {
      Response resp = given().auth().oauth2(tokenAdminBravoCharlie).when()
      .queryParam("filter", RoleStatus.PENDING.toString().toLowerCase())
      .get("/admin/provider/registrations").then()
      .statusCode(200)
      .body("type", equalTo(Urn.URN_SUCCESS.toString()))
      .body("title", equalTo(SUCC_TITLE_PROVIDER_REGS)).extract().response();
      
      JsonPath jpath = resp.jsonPath();
      
      String aliceBravoRegId = jpath.param("email", providerAliceEmail).param("url", BRAVO_SERVER)
          .get("results.find { it.email == email && it.rsUrl == url }.id");
      String bobBravoRegId = jpath.param("email", providerBobEmail).param("url", BRAVO_SERVER)
          .get("results.find { it.email == email && it.rsUrl == url }.id");
      String aliceCharlieRegId = jpath.param("email", providerAliceEmail).param("url", CHARLIE_SERVER)
          .get("results.find { it.email == email && it.rsUrl == url }.id");
      String bobCharlieRegId = jpath.param("email", providerBobEmail).param("url", CHARLIE_SERVER)
          .get("results.find { it.email == email && it.rsUrl == url }.id");
      
      JsonObject request = new JsonObject().put("request", new JsonArray()
          .add(new JsonObject().put("id", aliceBravoRegId).put("status",
              RoleStatus.APPROVED.toString().toLowerCase()))
          .add(new JsonObject().put("id", bobBravoRegId).put("status",
              RoleStatus.APPROVED.toString().toLowerCase()))
          .add(new JsonObject().put("id", aliceCharlieRegId).put("status",
              RoleStatus.REJECTED.toString().toLowerCase()))
          .add(new JsonObject().put("id", bobCharlieRegId).put("status",
              RoleStatus.REJECTED.toString().toLowerCase()))
          );
      ...
      given().auth().oauth2(tokenAdminBravoCharlie).contentType(ContentType.JSON)
        ...
        .body("results.name", everyItem(hasKey("firstName")))
        .body("results.name", everyItem(hasKey("lastName")))
        .body("results.find { it.email == '%s' && it.rsUrl == '%s'}.status",
    ...
    @Test
    @Order(10)
    @DisplayName("Bravo+Charlie admin checks pending, rejected and approved - Both providers in approved for Bravo, in rejected for Charlie")
    ...
    @Test
    @Order(11)
    @DisplayName("List roles after provider approve/reject - Alice has provider role for Alpha and Bravo. Bob has only for Bravo")
    void listingRolesAfterProvidersStatusChange() {
      ...
      .body("results.rolesToRsMapping.provider", containsInAnyOrder(ALPHA_SERVER, BRAVO_SERVER));
  ...
  class RejectedProviderScenarios
  {
    ...
    void setup(KcAdminInt kc) {
      ...
      JsonObject provReq = new JsonObject()
        .put("provider", new JsonArray(List.of(ALPHA_SERVER)));
      ...
      String regId = given().auth().oauth2(tokenAdminAlpha).when()
          .queryParam("filter", RoleStatus.PENDING.toString().toLowerCase())
          .get("/admin/provider/registrations").then()
          .statusCode(200)
          .extract()
      ...
      JsonObject request = new JsonObject().put("request",
          new JsonArray().add(new JsonObject().put("id", regId).put("status",
              RoleStatus.REJECTED.toString().toLowerCase()))
          );
    ...
    void gettingCredsFail() {
      ...
      given().auth().oauth2(providerToken).contentType(ContentType.JSON).body(body.toString()).when()
    ...
    void delegationsFail() {
      ...
      JsonObject delegReq =
          new JsonObject().put("request",
              new JsonArray().add(new JsonObject().put("userEmail", "email@email.com")
                  .put("resSerUrl", ALPHA_SERVER)
                  .put("role", Roles.PROVIDER.toString().toLowerCase())));
      
      given().auth().oauth2(providerToken).contentType(ContentType.JSON).body(delegReq.toString())
      ...
      given().auth().oauth2(providerToken).contentType(ContentType.JSON).body(deleteDelegReq.toString())
    ...
    JsonObject tokenBody = new JsonObject().put("itemId", ALPHA_SERVER)
        .put("itemType", ItemType.RESOURCE_SERVER.toString().toLowerCase())
        .put("role", Roles.PROVIDER.toString().toLowerCase());
    
      given().auth().oauth2(providerToken).contentType(ContentType.JSON).body(tokenBody.toString())
