...
public class DelegationsIT {
  private static String ALPHA_SERVER = RandomStringUtils.randomAlphabetic(10).toLowerCase() + ".com";
  private static String BRAVO_SERVER = RandomStringUtils.randomAlphabetic(10).toLowerCase() + ".com";
  ...
  static void setup(KcAdminInt kc) {
    ...
    given().auth().oauth2(kc.cosAdminToken).contentType(ContentType.JSON).body(rsReqAlpha.toString())
    ...
    given().auth().oauth2(kc.cosAdminToken).contentType(ContentType.JSON).body(rsReqBravo.toString())
        .when().post("/admin/resourceservers").then()
    ...
    given().auth().oauth2(kc.cosAdminToken).contentType(ContentType.JSON).body(rsReqDummy.toString())
        .when().post("/admin/resourceservers").then()
        .statusCode(describedAs("Setup - Created Dummy RS server", is(201)));
    
    // create consumer roles for Alpha and Bravo server 
    JsonObject consReq = new JsonObject().put("consumer", new JsonArray().add(ALPHA_SERVER).add(BRAVO_SERVER));
    
    given().auth().oauth2(consumerToken).contentType(ContentType.JSON).body(consReq.toString()).when()
        .post("/user/roles").then()
        .statusCode(describedAs("Setup - Added consumer roles for Alpha, Bravo RS Server", is(200)));
    
    // create provider roles for Alpha and Bravo server 
    JsonObject provReq = new JsonObject().put("provider", new JsonArray().add(ALPHA_SERVER).add(BRAVO_SERVER));
    
    given().auth().oauth2(providerToken).contentType(ContentType.JSON).body(provReq.toString()).when()
        .post("/user/roles").then()
        .statusCode(describedAs("Setup - Added provider roles for Alpha, Bravo RS Server", is(200)));
    
    Response response =  
    given().auth().oauth2(tokenRoles).contentType(ContentType.JSON).when()
        .get("/admin/provider/registrations").then()
        .statusCode(describedAs("Setup - Get provider reg. ID", is(200))).extract().response();
    
    String provAlphaId =
        response.jsonPath().param("email", providerEmail).param("rsUrl", ALPHA_SERVER)
            .getString("results.find { it.email == email && it.rsUrl == rsUrl}.id");
    String provBravoId =
        response.jsonPath().param("email", providerEmail).param("rsUrl", BRAVO_SERVER)
            .getString("results.find { it.email == email && it.rsUrl == rsUrl}.id");
  ...
  void createDelegationRolesCannotCall(KcAdminInt kc)
  {
    ...
    given().auth().oauth2(kc.cosAdminToken).contentType(ContentType.JSON).body(delegReq.toString()).when()
    .post("/delegations").then()
    .statusCode(401)
    .body("type", equalTo(Urn.URN_INVALID_ROLE.toString()))
    .body("title", equalTo(ERR_TITLE_INVALID_ROLES))
  ...
  {
    given().auth().oauth2(providerToken).contentType(ContentType.JSON).when()
    .post("/delegations").then()
    .statusCode(400)
  ...
  void createDelegationUserNoHaveDelegatedRole()
  {
    ...
    given().auth().oauth2(providerToken).contentType(ContentType.JSON).body(delegReq.toString()).when()
    .post("/delegations").then()
    .statusCode(400)
    .body("type", equalTo(Urn.URN_INVALID_INPUT.toString()))
    .body("title", equalTo(ERR_TITLE_RS_NOT_EXIST_OR_USER_NO_HAVE_ROLE))
  ...
  void createDelegationEmailUserNoHaveRoleForDelegatedRs()
  {
    ...
    given().auth().oauth2(consumerToken).contentType(ContentType.JSON).body(consumerDelegReq.toString()).when()
    .post("/delegations").then()
    .statusCode(400)
    .body("type", equalTo(Urn.URN_INVALID_INPUT.toString()))
    .body("title", equalTo(ERR_TITLE_RS_NOT_EXIST_OR_USER_NO_HAVE_ROLE))
    .body("detail", equalTo(ERR_DETAIL_RS_NOT_EXIST_OR_USER_NO_HAVE_ROLE))
    .body("context." + ERR_CONTEXT_RS_NOT_EXIST_OR_USER_NO_HAVE_ROLE, contains(DUMMY_SERVER_NO_ONE_HAS_ROLES));
    ...
    given().auth().oauth2(providerToken).contentType(ContentType.JSON).body(providerDelegReq.toString()).when()
    .post("/delegations").then()
    .statusCode(400)
    .body("type", equalTo(Urn.URN_INVALID_INPUT.toString()))
    .body("title", equalTo(ERR_TITLE_RS_NOT_EXIST_OR_USER_NO_HAVE_ROLE))
  ...
  void createDelegationEmailNotExist()
  {
    ...
    JsonObject delegReq = new JsonObject().put("request",
        new JsonArray().add(new JsonObject()
            ...
            .put("resSerUrl", BRAVO_SERVER).put("role", Roles.CONSUMER.toString().toLowerCase()))
        .add(new JsonObject()
  ...
  {
    given().auth().oauth2(tokenRoles).when()
    .get("/delegations").then()
    .statusCode(401)
    .body("type", equalTo(Urn.URN_INVALID_ROLE.toString()))
    .body("title", equalTo(ERR_TITLE_INVALID_ROLES))
  ...
  {
    given().auth().oauth2(tokenNoRoles).when()
    .get("/delegations").then()
    .statusCode(401)
    .body("type", equalTo(Urn.URN_INVALID_ROLE.toString()))
    .body("title", equalTo(ERR_TITLE_INVALID_ROLES))
  ...
  {
    JsonObject deleteReq = new JsonObject().put("request",
        new JsonArray().add(new JsonObject()
            .put("id", UUID.randomUUID().toString())));
    
    given().auth().oauth2(tokenRoles).contentType(ContentType.JSON).body(deleteReq.toString()).when()
    .delete("/delegations").then()
    .statusCode(401)
    .body("type", equalTo(Urn.URN_INVALID_ROLE.toString()))
    .body("title", equalTo(ERR_TITLE_INVALID_ROLES))
    .body("detail", equalTo(ERR_DETAIL_DEL_DELEGATE_ROLES));
    
    given().auth().oauth2(kc.cosAdminToken).contentType(ContentType.JSON).body(deleteReq.toString()).when()
    .delete("/delegations").then()
    .statusCode(401)
    .body("type", equalTo(Urn.URN_INVALID_ROLE.toString()))
    .body("title", equalTo(ERR_TITLE_INVALID_ROLES))
  ...
  void deleteDelegationNoRolesCannotCall()
  {
    ...
    given().auth().oauth2(tokenNoRoles).contentType(ContentType.JSON).body(deleteReq.toString()).when()
    .delete("/delegations").then()
    .statusCode(401)
    .body("type", equalTo(Urn.URN_INVALID_ROLE.toString()))
    .body("title", equalTo(ERR_TITLE_INVALID_ROLES))
  ...
  void deleteDelegationMissingToken()
  {
    ...
    given().contentType(ContentType.JSON).body(deleteReq.toString()).when()
    .delete("/delegations").then()
    .statusCode(401)
    .body("type", equalTo(Urn.URN_MISSING_AUTH_TOKEN.toString()));
  }
  ...
  void deleteDelegationInvalidMissingKeys()
  {
    ...
    given().auth().oauth2(providerToken).contentType(ContentType.JSON).body(deleteReq.toString()).when()
    .delete("/delegations").then()
    .statusCode(400)
    .body("type", equalTo(Urn.URN_INVALID_INPUT.toString()));
    
   deleteReq = new JsonObject().put("request",
        new JsonArray().add(new JsonObject()
            .put("something", UUID.randomUUID().toString())));
   ...
   deleteReq = new JsonObject().put("request",
        new JsonArray().add(new JsonObject()
            .put("id", RandomStringUtils.random(20))));
  ...
  void userBothConsProvRole(KcAdminInt kc)
  {
    ...
    JsonObject consReq = new JsonObject().put("provider", new JsonArray().add(ALPHA_SERVER))
        .put("consumer", new JsonArray().add(BRAVO_SERVER));
    
    given().auth().oauth2(provConsToken).contentType(ContentType.JSON).body(consReq.toString()).when()
        .post("/user/roles").then()
        .statusCode(describedAs("Setup - Added provider role for Alpha RS Server, consumer for Bravo", is(200)));
    ...
    String provAlphaId =
        response.jsonPath().param("email", provConsEmail).param("rsUrl", ALPHA_SERVER)
            .getString("results.find { it.email == email && it.rsUrl == rsUrl}.id");
    
    JsonObject approveReq = new JsonObject().put("request", new JsonArray().add(new JsonObject()
        .put("status", RoleStatus.APPROVED.toString().toLowerCase()).put("id", provAlphaId)));
    ...
    JsonObject delegReq =
        new JsonObject().put("request",
            new JsonArray()
            .add(new JsonObject().put("userEmail", delegateEmail)
                .put("resSerUrl", ALPHA_SERVER)
    ...
    given().auth().oauth2(provConsToken).contentType(ContentType.JSON)
    .body(delegReq.toString())
    .when().post("/delegations").then().statusCode(201)
    .body("type", equalTo(Urn.URN_SUCCESS.toString()));
    
    given().auth().oauth2(provConsToken).contentType(ContentType.JSON)
    .when().get("/delegations").then().statusCode(200)
    .body("type", equalTo(Urn.URN_SUCCESS.toString()))
    .body("title", equalTo(SUCC_TITLE_LIST_DELEGS))
    .body("results.findAll { it.url == '%s' && it.role == '%s'}.user.email",
        withArgs(ALPHA_SERVER, Roles.PROVIDER.toString().toLowerCase()), containsInAnyOrder(delegateEmail))
    .body("results.findAll { it.url == '%s' && it.role == '%s'}.user.email",
        withArgs(BRAVO_SERVER, Roles.CONSUMER.toString().toLowerCase()), containsInAnyOrder(delegateEmail));
  }
  ...
  class DelegationLifecycle {
    ...
    {
      given().auth().oauth2(providerToken).when()
      .get("/delegations").then()
      .statusCode(200)
      .body("type", equalTo(Urn.URN_SUCCESS.toString()))
      .body("title", equalTo(SUCC_TITLE_LIST_DELEGS))
      .body("results.user.email", not(containsInAnyOrder(delegateAliceEmail, delegateBobEmail)));
      
      given().auth().oauth2(consumerToken).when()
      .get("/delegations").then()
      .statusCode(200)
      .body("type", equalTo(Urn.URN_SUCCESS.toString()))
      .body("title", equalTo(SUCC_TITLE_LIST_DELEGS))
    ...
    {
      given().auth().oauth2(delegateAliceToken).when()
      .get("/delegations").then()
      .statusCode(401)
      .body("type", equalTo(Urn.URN_INVALID_ROLE.toString()))
      .body("title", equalTo(ERR_TITLE_INVALID_ROLES))
      ...
      given().auth().oauth2(delegateBobToken).when()
      .get("/delegations").then()
      .statusCode(401)
      .body("type", equalTo(Urn.URN_INVALID_ROLE.toString()))
      .body("title", equalTo(ERR_TITLE_INVALID_ROLES))
    ...
    void createConsumerDelegateSuccess()
    {
      JsonObject delegReq =
          new JsonObject().put("request",
              new JsonArray().add(new JsonObject().put("userEmail", delegateAliceEmail)
                  .put("resSerUrl", ALPHA_SERVER)
                  .put("role", Roles.CONSUMER.toString().toLowerCase())));
    
      given().auth().oauth2(consumerToken).contentType(ContentType.JSON)
      .body(delegReq.toString())
      .when().post("/delegations").then().statusCode(201)
      .body("type", equalTo(Urn.URN_SUCCESS.toString()));
    }
    ...
    void createProviderDelegateSuccess()
    {
      ...
      given().auth().oauth2(providerToken).contentType(ContentType.JSON)
      .body(delegReq.toString())
      .when().post("/delegations").then().statusCode(201)
      .body("type", equalTo(Urn.URN_SUCCESS.toString()));
    }
    ...
    @Test
    @Order(5)
    @DisplayName("Consumer and provider lists delegates - delegations exist for Alice for Alpha server")
    void delegatorListDelegates()
    {
      ...
      .body("type", equalTo(Urn.URN_SUCCESS.toString()))
      .body("title", equalTo(SUCC_TITLE_LIST_DELEGS))
          .body("results.find { it.url == '%s' && it.role == '%s' && it.user.email == '%s'}",
    ...
    @Test
    @Order(6)
    @DisplayName("Delegates list delegations - consumer delegation and provider delegation exists Alpha server for Alice, "
        + "Bob gets 401")
    void delegateListDelegates()
    {
      given().auth().oauth2(delegateAliceToken).contentType(ContentType.JSON)
      .when().get("/delegations").then().statusCode(200)
      .body("type", equalTo(Urn.URN_SUCCESS.toString()))
      .body("title", equalTo(SUCC_TITLE_LIST_DELEGS))
          .body("results.find { it.url == '%s' && it.role == '%s' }",
              withArgs(ALPHA_SERVER, Roles.CONSUMER.toString().toLowerCase(), consumerEmail),
              not(nullValue()))
          .body("results.find { it.url == '%s' && it.role == '%s' && it.owner.email == '%s'}",
      ...
      given().auth().oauth2(delegateBobToken).contentType(ContentType.JSON)
      .when().get("/delegations").then().statusCode(401)
      .body("type", equalTo(Urn.URN_INVALID_ROLE.toString()));
    }

    @Test
    @Order(7)
    @DisplayName("Consumer tries setting Alice as delegate for Bravo AND Alpha in same request - fails because latter exists")
    void consumerDelegateAlreadyExists()
    {
      String consDelegIdAliceAlpha = given().auth().oauth2(consumerToken).contentType(ContentType.JSON)
      .when().get("/delegations").then().statusCode(200)
      .body("type", equalTo(Urn.URN_SUCCESS.toString()))
      .body("title", equalTo(SUCC_TITLE_LIST_DELEGS))
          .extract().path("results.find { it.url == '%s' && it.role == '%s' && it.user.email == '%s'}.id",
              ALPHA_SERVER, Roles.CONSUMER.toString().toLowerCase(), delegateAliceEmail);
      
      JsonObject delegReq =
          new JsonObject().put("request",
              new JsonArray()
              .add(new JsonObject().put("userEmail", delegateAliceEmail)
                  .put("resSerUrl", ALPHA_SERVER)
                  .put("role", Roles.CONSUMER.toString().toLowerCase()))
              .add(new JsonObject().put("userEmail", delegateAliceEmail)
                  .put("resSerUrl", BRAVO_SERVER)
    ...
    @Test
    @Order(8)
    @DisplayName("Provider tries setting Alice as delegate for Bravo AND Alpha in same request - fails because latter exists")
    void providerDelegateAlreadyExists()
    {
      String provDelegIdAliceAlpha = given().auth().oauth2(providerToken).contentType(ContentType.JSON)
      .when().get("/delegations").then().statusCode(200)
      .body("type", equalTo(Urn.URN_SUCCESS.toString()))
      .body("title", equalTo(SUCC_TITLE_LIST_DELEGS))
          .extract().path("results.find { it.url == '%s' && it.role == '%s' && it.user.email == '%s'}.id",
    ...
    @Test
    @Order(9)
    @DisplayName("Provider creates Alice and Bob as provider delegate for Bravo server - repeat fails")
    void createProviderDelegateMultipleSuccess()
    {
      JsonObject delegReq =
          new JsonObject().put("request",
              ...
              .add(new JsonObject().put("userEmail", delegateAliceEmail)
                  .put("resSerUrl", BRAVO_SERVER)
                  .put("role", Roles.PROVIDER.toString().toLowerCase()))
              .add(new JsonObject().put("userEmail", delegateBobEmail)
                  .put("resSerUrl", BRAVO_SERVER)
                  .put("role", Roles.PROVIDER.toString().toLowerCase()))
              );
      ...
      given().auth().oauth2(providerToken).contentType(ContentType.JSON)
      .body(delegReq.toString())
      .when().post("/delegations").then().statusCode(409)
      .body("type", equalTo(Urn.URN_ALREADY_EXISTS.toString()));
    }
    
    @Test
    @Order(10)
    @DisplayName("Consumer creates Alice and Bob as consumer delegate for Bravo server - repeat fails")
    ...
    @Test
    @Order(11)
    @DisplayName("Consumer and provider lists delegates - delegations exist for Alice for Alpha, Bravo server and "
    ...
    void delegatorListDelegatesAfterMultiInsert()
    {
      ...
      .body("type", equalTo(Urn.URN_SUCCESS.toString()))
      .body("title", equalTo(SUCC_TITLE_LIST_DELEGS))
      .body("results.findAll { it.role == '%s' && it.user.email == '%s'}.url",
    ...
    @Test
    @Order(12)
    @DisplayName("Alice and Bob lists delegations - delegations exist for Alice for Alpha, Bravo server and "
        + "Bob for Bravo server")
    void delegatesListDelegatesAfterMultiInsert()
    {
      ...
      .body("type", equalTo(Urn.URN_SUCCESS.toString()))
      .body("title", equalTo(SUCC_TITLE_LIST_DELEGS))
      .body("results.findAll { it.role == '%s' && it.owner.email == '%s' }.url",
    ...
    @Test
    @Order(16)
    @DisplayName("Consumer and provider lists delegates - delegations gone for Alice for Alpha, still for Bravo server and "
        + "Bob for Bravo server")
    ...
    {
      Response resp = given().auth().oauth2(consumerToken).contentType(ContentType.JSON)
      .when().get("/delegations").then().statusCode(200)
          .extract().response();
      
      String aliceBravoId = resp.jsonPath().param("email", delegateAliceEmail)
          .param("role", Roles.CONSUMER.toString().toLowerCase()).param("url", BRAVO_SERVER)
          .getString(
              "results.find { it.url == url && it.role == role && it.user.email == email}.id");
      String bobBravoId = resp.jsonPath().param("email", delegateBobEmail)
          .param("role", Roles.CONSUMER.toString().toLowerCase()).param("url", BRAVO_SERVER)
          .getString(
              "results.find { it.url == url && it.role == role && it.user.email == email}.id");
    ...
    {
      Response resp = given().auth().oauth2(providerToken).contentType(ContentType.JSON)
      .when().get("/delegations").then().statusCode(200)
          .extract().response();
      
      String aliceBravoId = resp.jsonPath().param("email", delegateAliceEmail)
          .param("role", Roles.PROVIDER.toString().toLowerCase()).param("url", BRAVO_SERVER)
          .getString(
              "results.find { it.url == url && it.role == role && it.user.email == email}.id");
      String bobBravoId = resp.jsonPath().param("email", delegateBobEmail)
          .param("role", Roles.PROVIDER.toString().toLowerCase()).param("url", BRAVO_SERVER)
    ...
    @Test
    @Order(20)
    @DisplayName("Provider and consumer user list delegations after deleting - no delegations for Alice, Bob")
  ...
  @TestInstance(Lifecycle.PER_CLASS)
  @TestMethodOrder(OrderAnnotation.class)
  @DisplayName("Delegate lifecycle as delegations are created and deleted + Delegation ID for token requests tests")
  class DelegateLifecycleAndDelegationIdTest {
    ...
    void listRolesAfterCreate()
    {
      given().auth().oauth2(delegateAliceToken).when()
          ...
          .body("results.rolesToRsMapping.delegate", containsInAnyOrder(ALPHA_SERVER, BRAVO_SERVER));
    ...
    @Test
    @Order(5)
    @DisplayName("Delegation ID test - Alice gets provider RS token for Alpha, Bob gets consumer RS token for Bravo")
    void delegIdTestSuccess()
    {
      String provDelegIdAliceAlpha = given().auth().oauth2(delegateAliceToken)
          .contentType(ContentType.JSON).when().get("/delegations").then().statusCode(200).extract()
          .path("results.find { it.url == '%s' && it.role == '%s' && it.owner.email == '%s'}.id",
              ALPHA_SERVER, Roles.PROVIDER.toString().toLowerCase(), providerEmail);

      String consDelegIdBobBravo = given().auth().oauth2(delegateBobToken)
          .contentType(ContentType.JSON).when().get("/delegations").then().statusCode(200).extract()
          .path("results.find { it.url == '%s' && it.role == '%s' && it.owner.email == '%s'}.id",
              BRAVO_SERVER, Roles.CONSUMER.toString().toLowerCase(), consumerEmail);
      
      JsonObject aliceTokenReq = new JsonObject().put("itemId", ALPHA_SERVER)
          .put("itemType", ItemType.RESOURCE_SERVER.toString().toLowerCase())
          .put("role", Roles.DELEGATE.toString().toLowerCase());
      
      JsonObject bobTokenReq = new JsonObject().put("itemId", BRAVO_SERVER)
          .put("itemType", ItemType.RESOURCE_SERVER.toString().toLowerCase())
          .put("role", Roles.DELEGATE.toString().toLowerCase());
     
      given().auth().oauth2(delegateAliceToken).header("delegationId", provDelegIdAliceAlpha)
      .contentType(ContentType.JSON).body(aliceTokenReq.toString())
      .when().post("/token").then().statusCode(200)
      .body("type", equalTo(Urn.URN_SUCCESS.toString()));
      
      given().auth().oauth2(delegateBobToken).header("delegationId", consDelegIdBobBravo)
      .contentType(ContentType.JSON).body(bobTokenReq.toString())
      .when().post("/token").then().statusCode(200)
      .body("type", equalTo(Urn.URN_SUCCESS.toString()));
    }
    ...
    void delegIdTestNonExistDelegId()
    {
      ...
      given().auth().oauth2(delegateAliceToken).header("delegationId", UUID.randomUUID().toString())
      .contentType(ContentType.JSON).body(aliceTokenReq.toString())
    ...
    void delegIdTestInvalidDelegId()
    {
      ...
      given().auth().oauth2(delegateAliceToken).header("delegationId", RandomStringUtils.random(10))
      .contentType(ContentType.JSON).body(aliceTokenReq.toString())
      .when().post("/token").then().statusCode(400)
      .body("type", equalTo(Urn.URN_INVALID_INPUT.toString()));
      
      given().auth().oauth2(delegateAliceToken).header("delegationId", "")
      .contentType(ContentType.JSON).body(aliceTokenReq.toString())
      .when().post("/token").then().statusCode(400)
      .body("type", equalTo(Urn.URN_INVALID_INPUT.toString()));
    }
    ...
    void delegIdTestNotOwned()
    {
      ...
      JsonObject bobTokenReq = new JsonObject().put("itemId", ALPHA_SERVER)
          .put("itemType", ItemType.RESOURCE_SERVER.toString().toLowerCase())
          .put("role", Roles.DELEGATE.toString().toLowerCase());
     
      given().auth().oauth2(delegateBobToken).header("delegationId", provDelegIdAliceAlpha)
      .contentType(ContentType.JSON).body(bobTokenReq.toString())
      .when().post("/token").then().statusCode(401)
      .body("type", equalTo(Urn.URN_INVALID_INPUT.toString()))
      .body("title", equalTo(ERR_DELEGATE))
    ...
    @Test
    @Order(9)
    @DisplayName("Get delegate emails - provider and consumer calls will return both delegates each")
    void getDelegateEmails()
    {
      String providerUserId = IntegTestHelpers.getUserIdFromKcToken(providerToken);
      Map<String, String> provQueryParams = Map.of("userId", providerUserId, "resourceServer",
              ALPHA_SERVER, "role", Roles.PROVIDER.toString().toLowerCase());
      
      String consumerUserId = IntegTestHelpers.getUserIdFromKcToken(consumerToken);
      Map<String, String> consQueryParams = Map.of("userId", consumerUserId, "resourceServer",
              BRAVO_SERVER, "role", Roles.CONSUMER.toString().toLowerCase());
      
      given().when().headers(rolesTrusteeClientCreds)
    ...
    @Test
    @Order(10)
    @DisplayName("Delegation ID test - Delete all delegations for Bob, delete provider deleg Alpha"
    ...
    void delegIdTestDeletedDelegationId()
    {
      ...
      String provDelegIdBobAlpha = given().auth().oauth2(delegateBobToken)
          .contentType(ContentType.JSON).when().get("/delegations").then().statusCode(200).extract()
          .path("results.find { it.url == '%s' && it.role == '%s' && it.owner.email == '%s'}.id",
              ALPHA_SERVER, Roles.PROVIDER.toString().toLowerCase(), providerEmail);
      ...
      JsonObject consDelegReq =
          new JsonObject().put("request",
              new JsonArray()
              .add(new JsonObject().put("id", consDelegIdBobBravo))
              );
    ...
    @Test
    @Order(12)
    @DisplayName("Get delegate emails - provider has no delegates anymore, so empty, consumer has Alice as delegate on Bravo")
