...
/**
 * Integration tests for APD creation, listing and status update. Also tests user getting trustee
 * role and lifecycle of role as associated APD is made active/inactive.
 */
...
public class ApdIT {
  ...
  @Test
  @DisplayName(
      "Create APD - User with consumer, provider, admin, delegate, trustee roles cannot call API")
                ...
                new JsonArray()
                    .add(
                        new JsonObject()
                            ...
                            .put("status", ApdStatus.ACTIVE.toString().toLowerCase())));
            ...
            .put(
                ...
                new JsonArray()
                    .add(
                        new JsonObject()
                            .put("id", randId)
                            .put("status", ApdStatus.INACTIVE.toString().toLowerCase())));
    ...
    JsonObject noId =
        new JsonObject()
            .put(
                ...
                new JsonArray()
                    .add(
                        new JsonObject().put("status", ApdStatus.ACTIVE.toString().toLowerCase())));
        ...
        new JsonObject()
            .put(
                ...
                new JsonArray()
                    .add(
                        new JsonObject()
                            .put("id", RandomStringUtils.randomAlphabetic(10))
                            .put("status", ApdStatus.ACTIVE.toString().toLowerCase())));

    JsonObject invalidStatus =
        new JsonObject()
            .put(
                "request",
                new JsonArray()
  ...
  class ApdStateChangingFlow {
    ...
    void setup(KcAdminInt kc) {
      ...
      JsonObject body =
          new JsonObject()
              .put("name", serverNameAlpha)
              .put("url", serverUrlAlpha)
              .put("owner", email);
      ...
      body =
          new JsonObject()
              .put("name", serverNameBravo)
              .put("url", serverUrlBravo)
              .put("owner", email);
    ...
    @Order(4)
    @DisplayName(
        "Trying to activate Server Bravo (wrong) and activate Server Alpha (correct) - fails")
    ...
    void multipleReqSuccess(KcAdminInt kc) {
      ...
      given()
          ...
          .body(
              "results.find {it.id == '%s'}.status",
              withArgs(serverIdAlpha), equalTo(ApdStatus.ACTIVE.toString().toLowerCase()))
          .body(
              "results.find {it.id == '%s'}.url",
              withArgs(serverIdBravo), equalTo(serverUrlBravo.toLowerCase()))
          .body(
              "results.find {it.id == '%s'}.status",
              withArgs(serverIdBravo), equalTo(ApdStatus.INACTIVE.toString().toLowerCase()));
    }
  ...
  class ApdAndTrusteeFlow {
    ...
    void setup(KcAdminInt kc) {
      ...
      given()
          ...
          .then()
          .statusCode(
              describedAs("Setup - Added consumer role to trustee to get client creds", is(200)));
    ...
    void userCannotCallSearchAndGetDelegEmailApis() {
      given()
          .when()
          .headers(Map.of("clientId", trusteeClientId, "clientSecret", trusteeClientSecret))
          .queryParams(
              Map.of(
                  "userId",
    ...
    @Order(2)
    @DisplayName(
        "User going to be trustee does not have trustee role (has consumer role, so won't get 404)")
